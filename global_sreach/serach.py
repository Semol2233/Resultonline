    ...
    def get(self,request,query):
        ...
        if filter_postmodel_q:
            for p in filter_postmodel_q:
                ...
                result.append(p)
        filter_PostCreate = PostCreate.objects.filter(Q(title__contains=query) | Q(details__contains=query)).values('title','details','photo','slug','is_active')
        if filter_PostCreate:
            for b in filter_PostCreate:
                ...
                result.append(b)
        filter_postmodel = postmodel.objects.filter(Q(title__contains=query) | Q(details__contains=query)).values('title','details','photo','slug','is_active','tag_creator')
        if filter_postmodel:
