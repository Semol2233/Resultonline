...
class PaginatedProjectsAPIView(APIView, PaginationHandlerMixin):
    ...
    def get(self, request, category, *args, **kwargs):
        authors = cat_model_q.objects.filter(q_slug=category).values('publisher','q_slug','is_active')
        if authors:
            posts = postmodel_q.objects.filter(catagry__q_slug=category).values('title', 'slug', 'details','view','created_at','is_active')
            for author in list(authors):
                response = {
...
class qanda_root(generics.ListAPIView):
    queryset               = cat_model_q.objects.filter(created_date__gte=datetime.now() - timedelta(days=55))
    serializer_class       = cat_modelSrtilizer
    pagination_class       = qa_pagenasssstion
